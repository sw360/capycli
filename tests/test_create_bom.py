...
class TestCreateBom(TestBasePytest):
    ...
    def test_project_by_id(self):
        ...
        assert ext_refs[0].url == release["sourceCodeDownloadurl"]
        ...
        assert ext_refs[0].url == release["_embedded"]["sw360:attachments"][0]["filename"]
        assert ext_refs[0].type == ExternalReferenceType.DISTRIBUTION
        ...
        assert len(ext_refs) == 1
        assert ext_refs[0].url == release["repository"]["url"]
